...
# for i in range(len(lda)):
#     if i >= 6:
#         ld_19a.append(lda[i-6])
#     else:
#         ld_19a.append(np.mean(lda[:6]))
#
#
# df['before_36hr_wdir'] = pd.DataFrame(ld_17a)
...
#     if sub.iloc[:,'Accuracy'] <=50 :
#         sub = sub.drop([i], axis=0)
#     else:
#         continue
...
#print(sub.describe())
print("The accuracy of Model is : " ,sub['Accuracy'].mean())
...
#plt.axes.Axes.set_xscale(0, 'linear')
#plt.axis([0, 60, 0, 0.03])
plt.title("Prediction and observation of wind speed for every 6 hours")

plt.xlabel("Hours on the scale of 6")
